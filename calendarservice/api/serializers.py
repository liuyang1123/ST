...
class EventSerializer(serializers.Serializer):
    id = serializers.CharField(max_length=36, read_only=True) # autogenerated by rethinkdb
    calendar_id = serializers.CharField(max_length=36, read_only=True) # self.kwargs['calendar_id']
    user_id = serializers.CharField(max_length=36, read_only=True)
    summary = serializers.CharField(max_length=140, allow_blank=True, default="")
    description = serializers.CharField(max_length=200, allow_blank=True, default="")
    deleted = serializers.BooleanField(default=False)
    # start = serializers.DateTimeField(allow_null=True, default=None) # Can be blank = To be determined
    start = serializers.CharField(max_length=50, allow_blank=True, default="")
    # end = serializers.DateTimeField(allow_null=True, default=None) # Can be blank = To be determined
    end = serializers.CharField(max_length=50, allow_blank=True, default="") # Can be blank = To be determined
    duration = serializers.IntegerField(allow_null=True, required=False) # start - end or a specific duration when start y end are null
    location = serializers.CharField(max_length=140, allow_blank=True, default="")
    participation_status = serializers.CharField(max_length=20, default=UNKNOWN, allow_blank=True)
    attendees = serializers.ListField(allow_null=True)
    transparency = serializers.CharField(max_length=20, default=UNKNOWN, allow_blank=True)
    event_status = serializers.CharField(max_length=20, default=UNKNOWN, allow_blank=True)
    categories = serializers.CharField(max_length=20, allow_blank=True, default="Meeting")
    is_fixed = serializers.BooleanField(default=False)
    created = serializers.DateTimeField(read_only=True) # Set on inserted
    updated = serializers.DateTimeField(read_only=True) # Set on inserted, and update
